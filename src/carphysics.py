    ...
    def process(self):
        for ent, (accel, dire, velo, ff, eng, grb, cha, dt, cvelo) in self.world.get_components(com.Acceleration, com.Direction, com.Velocity, com.ForwardForce, com.Engine, com.GearBox, com.Chassis, com.DeltaTime, com.CarVelocity):
            ...
            if grb.gear_ratios[grb.current_gear] == 0 or grb.clutch == True:
                ...
                ###### CODE FOR THE CLUTCH MATING TO THE FLYWHEEL
            ...
            """
            # Calculates the engine RPM from speed
            eng.rpm = (abs(cvelo.velV.x)/cha.wheel_radius) * (grb.rear_diff[0] * abs(grb.gear_ratios[grb.current_gear])) * constants.RADS_to_RPM
            # Manages the idle RPM
            if eng.rpm < eng.idle:
                eng.rpm = eng.idle
...
class DrivetrainProcessor(esper.Processor):
    def process(self):
            if grb.clutch == False:
                ...
                eng_inertia = 0.5 * 9.34 * 0.16**2 # 0.5 * mass[0] * radius^2
                temp_rpm = eng.rpm
                ...
                grb.clutch_rpm = (velo.velV.magnitude() * grb.rear_diff[0] * 3.6) / (cha.wheel_diameter/2) * constants.RADS_to_RPM
                
                #temp_clutch_rpm = grb.clutch_rpm
                #clutch_acc = (velo.velV.magnitude() * grb.rear_diff[0] * grb.gear_ratio) / (cha.wheel_diameter/2) * constants.RADS_to_RPM
    ...
    def process(self):
        for ent, (eng, grb, cha, velo, dt) in self.world.get_components(com.Engine, com.GearBox, com.Chassis, com.Velocity, com.DeltaTime):
            if grb.gear_ratios[grb.current_gear] == 0 or grb.clutch == True:
                ...
                eng.rpm = int((velo.velV.magnitude()/cha.wheel_diameter/2) * (grb.rear_diff[0] * grb.gear_ratios[grb.current_gear]) * constants.RADS_to_RPM)
                ...
                cha.drive_torque = eng_torque * eng.throttle * grb.rear_diff[0] * grb.gear_ratios[grb.current_gear]
